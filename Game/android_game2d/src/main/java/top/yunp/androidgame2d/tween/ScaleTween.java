...
import top.yunp.lib.java.lang.Math;
...
public class ScaleTween extends Tween {
    ...
    protected void onEnterFrame(int currentFrame, int frames) {
        if (speesX != 0) {
            getTarget().setScaleX(getStartScaleX() + speesX * currentFrame);
        }
        if (speedY != 0) {
    ...
    private void computeScaleXDistancePerFrame() {
        speesX = ((float) (endScaleX - startScaleX)) / getFrames();
    }
    ...
    private float startScaleX = 1, startScaleY = 1, endScaleX = 1, endScaleY = 1, speesX = 0, speedY = 0;
